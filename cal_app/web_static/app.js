...
function renderStatusEditor(item, prefix, isOverdue = false) {
  const key = `${prefix}-${scheduleKey(item)}`;
  const delayBtn = isOverdue
    ? `<button class="mini-btn" data-delay-task="${item.task_id}">逾期到今天</button>`
    : "";
  return `
...
function renderOverdue() {
  ...
  for (const item of state.overdueSchedules) {
    ...
    tr.innerHTML = `
      ...
      <td>${item.start_date} -> ${item.end_date}</td>
      <td><span class="status-pill status-${item.status}">${item.status}</span></td>
      <td>${renderStatusEditor(item, "overdue", true)}</td>
    `;
...
function setCreationDefaults() {
  if (!state.today) return;
  const onceStart = document.getElementById("createOneStart");
  const onceEnd = document.getElementById("createOneEnd");
  const recFirstStart = document.getElementById("createFirstStart");
  const recFirstEnd = document.getElementById("createFirstEnd");
  if (onceStart) onceStart.value = state.today;
  if (onceEnd) onceEnd.value = state.today;
  if (recFirstStart) recFirstStart.value = state.today;
  if (recFirstEnd) recFirstEnd.value = state.today;
}
...
document.body.addEventListener("click", async (event) => {
  ...
  const delayTask = button.dataset.delayTask;
  if (delayTask) {
    setBusy(true);
    try {
      await api("api/tasks/delay-one-time", { task_id: delayTask });
      await refresh();
      toast("已延期到今天");
    } catch (error) {
      toast(error.message, "error");
